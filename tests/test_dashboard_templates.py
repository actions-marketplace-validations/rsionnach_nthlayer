...
class TestPostgreSQLTemplate:
    ...
    def test_generates_panels(self):
        ...
        assert len(panels) == 12  # Should have 12 panels
        ...
        assert "Query Duration (p95)" in panel_titles
    ...
    def test_overview_panels_subset(self):
        ...
        assert "Cache Hit Ratio" in overview_titles
    
    def test_panels_have_targets(self):
        """Test that all panels have Prometheus targets."""
        template = PostgreSQLTemplate()
        panels = template.get_panels()
        ...
        for panel in panels:
            ...
            for target in panel.targets:
                assert target.expr  # Has PromQL query
...
class TestHTTPAPITemplate:
    ...
    def test_key_panels_present(self):
        ...
        assert "Response Time Percentiles" in panel_titles
        assert "Status Code Distribution" in panel_titles
    
    def test_uses_http_metrics(self):
        """Test that panels use HTTP metrics."""
        template = HTTPAPITemplate()
        panels = template.get_panels()
        ...
        for panel in panels:
            for target in panel.targets:
                if "http_request" in target.expr:
...
class TestTemplateIntegration:
    ...
    def test_templates_avoid_duplicates(self):
        ...
        postgres_panels = [p for p in dep_row.panels if "PostgreSQL" in p.title or "Cache Hit" in p.title or "Query Duration" in p.title]
