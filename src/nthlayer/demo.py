...
def build_parser() -> argparse.ArgumentParser:
    ...
    # reslayer collect
    collect_parser = reslayer_subparsers.add_parser(
    ...
    record_parser = reslayer_subparsers.add_parser(
        "record-deploy", help="Record a deployment manually"
    )
    ...
    # reslayer correlate
    correlate_parser = reslayer_subparsers.add_parser(
        "correlate", help="Correlate deployments with error budget burns"
    )
    ...
    # reslayer blame
    blame_parser = reslayer_subparsers.add_parser(
    ...
    alert_parser.add_argument(
        "--threshold", type=float, default=0.75, help="Budget threshold (0-1)"
    )
    alert_parser.add_argument(
        "--burn-rate", type=float, help="Burn rate threshold (e.g., 3.0 for 3x)"
    )
    ...
    # reslayer test-alert
    test_alert_parser = reslayer_subparsers.add_parser(
        "test-alert", help="Test alert notifications"
    )
    ...
    try:
        ...
        for slo in slos:
            ...
            if budget:
                ...
                print(
                    f"   Burned: {budget['budget']['burned_minutes']:.1f} minutes ({budget['budget']['percent_consumed']:.1f}%)"
                )
                print(
                    f"   Remaining: {budget['budget']['remaining_minutes']:.1f} minutes ({budget['budget']['percent_remaining']:.1f}%)"
                )
...
def demo_reslayer_collect(service: str, prometheus_url: str) -> None:
    ...
    try:
        ...
        for budget in budgets:
            ...
            print(
                f"   Period: {budget.period_start.strftime('%Y-%m-%d')} to {budget.period_end.strftime('%Y-%m-%d')}"
            )
...
def demo_reslayer_correlate(service: str, hours: int) -> None:
    ...
    try:
        ...
        if not results:
            ...
            print("Try:")
            print(
                f"  1. Record a deployment: nthlayer reslayer record-deploy {service} --commit <sha>"
            )
        ...
        if high:
            ...
            for r in high:
                print(
                    f"   • {r.deployment_id}: {r.burn_minutes:.1f} min burned ({r.confidence*100:.0f}%)"
                )
            print()
...
def demo_reslayer_blame(service: str, days: int, min_confidence: float) -> None:
    ...
    try:
        ...
        if not blamed:
            ...
            if all_deployments:
                print(
                    f"Found {len(all_deployments)} total deployment(s), but none correlated to burns."
                )
                print()
                print("Try:")
                print(
                    f"  • Lower threshold: nthlayer reslayer blame {service} --min-confidence 0.3"
                )
                print(f"  • Run correlation: nthlayer reslayer correlate {service}")
            else:
                ...
                print("Try:")
                print(
                    f"  • Record a deployment: nthlayer reslayer record-deploy {service} --commit <sha>"
                )
        ...
        medium = [
            d for d in blamed if MEDIUM_CONFIDENCE <= d.correlation_confidence < HIGH_CONFIDENCE
        ]
        ...
        if high:
            ...
            print(
                f"{'Commit':<12} {'Deployed':<20} {'Burned':<10} {'Confidence':<12} {'Author':<25}"
            )
            print("-" * 85)
        ...
        clean = [
            ...
            if not d.correlation_confidence or d.correlation_confidence < min_confidence
        ]
        ...
        print(
            f"  High Impact: {len(high)} ({len(high)*100//len(all_deployments) if all_deployments else 0}%)"
        )
        print(
            f"  Medium Impact: {len(medium)} ({len(medium)*100//len(all_deployments) if all_deployments else 0}%)"
        )
        print(
            f"  Clean: {len(clean)} ({len(clean)*100//len(all_deployments) if all_deployments else 0}%)"
        )
    ...
    try:
        ...
        if args.list:
            ...
            if not rules:
                ...
                print("To add a rule:")
                print(
                    f"  nthlayer reslayer alert-config {args.service} --threshold 0.75 --slack-webhook <url>"
                )
                return
...
def demo_reslayer_test_alert(
    service: str, slack_webhook: str | None, pagerduty_key: str | None
) -> None:
    ...
    try:
        ...
        if not slack_webhook and not pagerduty_key:
            ...
            print("Examples:")
            print(
                f"  nthlayer reslayer test-alert {service} --slack-webhook https://hooks.slack.com/..."
            )
